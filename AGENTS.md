...
- Product: native read-only vault reader; iOS first, Android second. Current scope is M1b plus the owner-requested Reading tab and local book/article resume; other M2/M3 features remain out of scope. Follow `docs/ARCHITECTURE.md` and record acceptance limits honestly in `docs/ACCEPTANCE.md`.
...
- Shared rendering lives in `packages/reader-web`; do not add WebKit/Android APIs there. Adapt hosts through `packages/contracts/reader-v2.d.ts`. Credentials never cross that bridge.
