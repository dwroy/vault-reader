...
Mac locking prevented additional manual desktop interaction, and the iPhone USB connection became unavailable during this follow-up. The previous build 4 remains the last confirmed installed phone version; the Reading extension's new device build/install status must be recorded separately. Synthetic simulator resume tests and real-file initial rendering do not claim a completed physical-device reading/touch pass or the full private-network/performance gates.
